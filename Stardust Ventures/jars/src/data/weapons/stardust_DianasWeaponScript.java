...
public class stardust_DianasWeaponScript implements EveryFrameWeaponEffectPlugin {
    // used Nia Tahl's Balor Weapon Scripts as the template for these
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        for (DamagingProjectileAPI proj : CombatUtils.getProjectilesWithinRange(weapon.getLocation(), 200f)) {
            if (proj.getWeapon() == weapon && !alreadyRegisteredProjectiles.contains(proj) && engine.isEntityInPlay(proj) && !proj.didDamage()) {
                engine.addPlugin(new stardust_DianasProjectileScript(proj, target));
